...
namespace dms {
	...
	std::vector<Point2f> getInputRegion(const ImageHead& in, RotateType type, const ObjectInfo& object, float expand_scale) {
		...
		float center_x = object.landmarks_2D[0].x;
		float center_y = object.landmarks_2D[0].y;
		float dx = object.landmarks_2D[1].x - object.landmarks_2D[0].x;
		float dy = object.landmarks_2D[1].y - object.landmarks_2D[0].y;
	...
	double YoloGetIOU(cv::Rect_<float> bb_test, cv::Rect_<float> bb_gt)
	{
	...
	void YoloNMS(std::vector<BoxInfo>& input_boxes, float NMS_THRESH) {
