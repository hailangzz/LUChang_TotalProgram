...
NPU_NMS_PARAM_S gstParam_yoloAll =
{
    ...
    100,    //u16KeepTopK;
    0.3,  //u16ConfidenceThreshold;

    0.4,    //u16NmsThreshold;
    0.4,    //u32OverlapThreshold;
    200,    //u16TopK;
    3,      //u16NumPath;

    {640, 640}, //au32InputSize
    3,      //u32NumAnchors
    {
        {{ 10.,  13.}, { 16.,  30.}, {  33.,  23.}},
        {{ 30.,  61.}, { 62.,  45.}, {  59., 119.}},
...
int read_image_file(std::string imgfilename, unsigned char* img, int format, int w, int h, int* sw, int* sx, int* sy)
{
	...
	v_orign_param.push_back(image.cols);
	v_orign_param.push_back(image.rows);
	v_orign_param.push_back(*sx);
	v_orign_param.push_back(*sy);
...
void SAMPLE_YoloDetect(VIM_F32 *pLocAddr[], VIM_F32 *pConfAddr[], VIM_U32 u32Lable)
{
    ...
    if(mparam.with_nms==1 && mparam.nms_load_flag == 0)
    {
        g_stParam = gstParam_yoloAll;
        g_pNmsHandle = Alg_Detect_Open(OPENMODE_EVALUATE);
	    mparam.nms_load_flag = 1;
    }
    
    for (unsigned int i=0; i<mparam.output_node.size(); i++)
        g_stParam.pLocAddr[i] = pLocAddr[i];

    Alg_Detect_YoloV5(g_pNmsHandle, &g_stParam, pDetData, &u32DetCnt); //解析：目标检测输出层的信息，包括NMS、坐标计算等

#if 1
    cv::Mat saveimg1 = v_saveimg[u32Lable];
    std::string imgsavename1 = v_imgsavename[u32Lable];
	int p_imgw = v_orign_param[u32Lable*4];  //当前输入图片的宽
	int p_imgh = v_orign_param[u32Lable*4+1];//当前输入图片的高
	int p_sx = v_orign_param[u32Lable*4+2];  //当前输入图片填充的宽
	int p_sy = v_orign_param[u32Lable*4+3];  //当前输入图片填充的高
    ...
    if (fop)
    {
        fprintf(fop, "No %d : %s\n", u32Lable+1, imgsavename1.c_str());
    }

    float score;
    char cidx[64];
    int label;
    for (i = 0; i < u32DetCnt; i++) //NMS操作后检测到的目标框个数
    {      
        score = pDetData[7*i+2];
        if (score > mparam.thresh)
        {
            label = (int)(pDetData[7*i+1]);
            x1 = (int)(pDetData[7*i+3]*p_imgw+0.5f)-p_sx; //将预测的坐标值，准换为实际原图的坐标值信息
            y1 = (int)(pDetData[7*i+4]*p_imgh+0.5f)-p_sy; 
            x2 = (int)(pDetData[7*i+5]*p_imgw+0.5f)-p_sx;
            y2 = (int)(pDetData[7*i+6]*p_imgh+0.5f)-p_sy;

            //printf("[%d]%d %d %d %d\n", i, x1, y1, x2, y2);

            cv::Rect rect(x1, y1, x2-x1, y2-y1);
            cv::rectangle(saveimg1, rect, cv::Scalar(255,0,0), 2, 1, 0);

            sprintf(cidx, "%d", label);
            std::string stridx = cidx;
            cv::putText(saveimg1, stridx, cv::Point(x1, y1-10), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0,0,255), 2);
            if (fop)
            {
                fprintf(fop, "[%d]label:%d,[%d,%d,%d,%d],%f\n", i,label, x1,y1,x2,y2,score);
            } 
        }  
     }
...
{
    param.modelpath = "model/yolov5s.npumodel";
    param.width = 640;
    param.height = 640;
    param.input_node = "input0";
    param.output_node.clear();
    param.output_node.push_back("Sigmoid199");
    param.output_node.push_back("Sigmoid201");
    param.output_node.push_back("Sigmoid203");
    param.format = RGB;
    param.with_nms = 1;
    param.nms_load_flag = 0;
    param.thresh = 0.3f;
