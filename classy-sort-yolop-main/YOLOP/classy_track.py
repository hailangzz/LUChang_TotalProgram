...
#yolov5
from test_onnx import infer_yolop
from lib.utils.general import check_img_size, non_max_suppression, scale_coords
...
def detect(opt, *args):
    ...
    names = model.module.names if hasattr(model, 'module') else model.names   # 获取检测目标的名称
    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset): #for every frame
        ...
        for i, det in enumerate(pred): #for each detection in this frame
            ...
            s += f'{img.shape[2:]}' #print images size and detection report
...
#infer_yolop(weight="yolop-640-640_origin.onnx")

if __name__ == '__main__':

    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str,
                        default='yolop-640-640_origin.onnx', help='model.pt path')

    args = parser.parse_args()
    boxes,img_bgr = infer_yolop(weight=args.weights) # nms后的predict box info
    ...
    for i, det in enumerate(boxes):  # for each detection in this frame # im0s为原始图像，其尺寸为 720* 1280
        ...
        # Rescale boxes from img_size (temporarily downscaled size) to im0 (native) size
        det[:, :4] = scale_coords(
            img.shape[2:], det[:, :4], img_bgr.shape).round()  # im0 为原始图像尺寸···
