...
def infer_yolop(weight="yolop-640-640.onnx",
    ...
    img_bgr = cv2.imread(img_path) # 读取原始图片
    height, width, _ = img_bgr.shape # 获取原始图片尺寸信息

    # convert to RGB
    img_rgb = img_bgr[:, :, ::-1].copy() # 图片存储格式转换

    # resize & normalize
    canvas, r, dw, dh, new_unpad_w, new_unpad_h = resize_unscale(img_rgb, (640, 640)) # 图片的padding resize 操作
