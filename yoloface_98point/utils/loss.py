...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            # Classification  # 这里将根据具体的关键点标注个数来修改，13=4+1+4*2（意思是说为检测框+置信度+4个关键点标注）
            if model.nc > 1:  # cls loss (only if multiple classes)
                t = torch.full_like(ps[:, 201:], cn, device=device)  # targets
                t[range(n), tcls[i]] = cp
                lcls += BCEcls(ps[:, 201:], t)  # BCE
            ...
            plandmarks = ps[:,5:201] #此处为关键点数据信息

            #将关键点标注信息同anchors框尺寸进行缩放的标准化处理。
            ...
            for point_index in range(point_number):
                plandmarks[:, point_index*2:point_index*2+2] = plandmarks[:, point_index*2:point_index*2+2] * anchors[i]
...
def build_targets(p, targets, model):
    ...
    gain = torch.ones(203, device=targets.device)
    ...
    for i in range(det.nl):
        ...
        #landmarks 10
        gain[6:202] = torch.tensor(p[i].shape)[[3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2]]  # xyxy gain
        ...
        # Append
        a = t[:, 202].long()  # anchor indices
        # indices.append((b, a, gj.clamp_(0, gain[3] - 1), gi.clamp_(0, gain[2] - 1)))  # image, anchor, grid indices
        indices.append(
            (b, a, gj.clamp_(0, int(gain[3] - 1)), gi.clamp_(0, int(gain[2] - 1))))  # image, anchor, grid indices
        ...
        point_number = 98
        for point_index in range(point_number):
            lks[:, [point_index*2, point_index*2+1]] = (lks[:, [point_index*2, point_index*2+1]] - gij)
