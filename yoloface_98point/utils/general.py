...
# import pkg_resources as pkg #自己加的
# import platform             #自己加的
...
# from utils.metrics import box_iou, fitnessimport pandas as pd
import pandas as pd
...
# def set_logging(name=None, verbose=True): #自己加的
...
#
# LOGGER = set_logging(__name__) #自己加的
#
#
# def check_suffix(file='yolov5s.pt', suffix=('.pt',), msg=''): #自己加的
...
#
# def check_python(minimum='3.6.2'): #自己加的
#     # Check current python version vs. required python version
#     check_version(platform.python_version(), minimum, name='Python ', hard=True)
#
#
# def check_version(current='0.0.0', minimum='0.0.0', name='version ', pinned=False, hard=False, verbose=False): #自己加的
...
def non_max_suppression_face(prediction, conf_thres=0.3, iou_thres=0.5, classes=None, agnostic=False, labels=()):
    """Performs Non-Maximum Suppression (NMS) on inference results
    Returns:
         detections with shape: nx6 (x1, y1, x2, y2, conf, cls)
    """

    nc = prediction.shape[2] - 201  # number of classes
    ...
    t = time.time()
    output = [torch.zeros((0, 202), device=prediction.device)] * prediction.shape[0]
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            l = labels[xi]
            v = torch.zeros((len(l), nc + 201), device=x.device)
            v[:, :4] = l[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
            v[range(len(l)), l[:, 0].long() + 201] = 1.0  # cls
            x = torch.cat((x, v), 0)
        ...
        # Compute conf
        x[:, 201:] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            i, j = (x[:, 201:] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, j + 201, None], x[i, 5:201] ,j[:, None].float()), 1)  #4,1,8,1,
        else:  # best class only
            conf, j = x[:, 201:].max(1, keepdim=True)
            x = torch.cat((box, conf, x[:, 5:201], j.float()), 1)[conf.view(-1) > conf_thres]
        ...
        # Batched NMS
        c = x[:, 201:202] * (0 if agnostic else max_wh)  # classes
