       ...
       # self.training |= self.export
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                class_range = list(range(5)) + list(range(201,201+self.nc))
                y[..., class_range] = x[i][..., class_range].sigmoid()   #这里是只对关键点以外的值进行sigmoid 包括：xywh、置信度、分类结果等
                y[..., 5:201] = x[i][..., 5:201]
                #y = x[i].sigmoid()
                ...
                #y[..., 5:15] = y[..., 5:15] * 8 - 4
                landmark_number=98
                for land_index in range(landmark_number):
                    y[..., 5+land_index*2:5+(land_index+1)*2] = y[..., 5+land_index*2:5+(land_index+1)*2] * self.anchor_grid[i] + self.grid[i].to(x[i].device) * self.stride[i]  # landmark x1 y1
