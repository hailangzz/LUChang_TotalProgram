...
def detect_one(model, image_path, device):
    ...
    if image_path == str(0):
        ...
        print(image_path)
        cv2.imwrite(image_path.split("\\")[-1].split('.')[0]+'.jpg', orgimg)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='runs/train/exp36/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--image', type=str, default=r'F:\AiTotalDatabase\CCPD2020\ccpd_green\val\test_images', help='source')  # file/folder, 0 for webcam
    ...
    all_image_path = os.listdir(opt.image)
    for single_image in all_image_path:
        single_image_path = os.path.join(opt.image,single_image)
        detect_one(model, single_image_path, device)
