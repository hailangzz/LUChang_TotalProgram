...
def detect(model, img0):
    ...
    gn_lks = torch.tensor(img0.shape)[[1, 0, 1, 0, 1, 0, 1, 0]].to(device)  # normalization gain landmarks
    boxes = []
    landmarks_list=[]
    h, w, c = img0.shape
    if pred is not None:
        ...
        for j in range(pred.size()[0]):
            ...
            for index in range(4):
                if index%2==0:
                    landmarks[index]=int(landmarks[index] * w)
                else:
                    landmarks[index] = int(landmarks[index] * h)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='runs/train/exp36/weights/last.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--dataset_folder', default=r'F:\AiTotalDatabase\CCPD2020\ccpd_green\val\test_images', type=str, help='dataset path')
    parser.add_argument('--folder_pict', default=r'F:\AiTotalDatabase\CCPD2020\ccpd_green\val\car_plate_val.txt', type=str, help='folder_pict')
    ...
    with torch.no_grad():
        ...
        for image_path in tqdm(glob.glob(os.path.join(testset_folder, '*'))):
            ...
            if img0 is None:
                ...
                continue
            boxes,landmarks = detect(model, img0)
            print(boxes)
            print(landmarks)
            # --------------------------------------------------------------------
            image_name = os.path.basename(image_path)
            txt_name = os.path.splitext(image_name)[0] + ".txt"
            save_name = os.path.join(opt.save_folder, txt_name)
            print(save_name)
