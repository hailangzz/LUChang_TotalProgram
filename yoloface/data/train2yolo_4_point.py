...
save_path = r'F:\AiTotalDatabase\人脸关键点数据集\WIDR_FACE\WIDER_train'
aa=WiderFaceDetection(r"F:\AiTotalDatabase\人脸关键点数据集\WIDR_FACE\retinaface_gt_v1.1\train/label.txt")
for i in range(len(aa.imgs_path)):
    ...
    save_txt_path = os.path.join(save_path,'labels_4_point', base_txt)
    with open(save_txt_path, "w") as f:
        height, width, _ = img.shape
        labels = aa.words[i]
        annotations = np.zeros((0, 12))
        if len(labels) == 0:
            continue
        for idx, label in enumerate(labels):
