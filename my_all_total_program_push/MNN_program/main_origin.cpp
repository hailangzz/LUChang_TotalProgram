...
cv::Mat draw_box(cv::Mat& cv_mat, std::vector<BoxInfo>& boxes, int CNUM)
{
    ...
    */
    ...
    for (auto box : boxes)
    {
        ...
        std::cout << class_names[box.label] <<" : " << box.score * 100 << std::endl;
        sprintf_s(text, "%s:%.1f%%", class_names[box.label], box.score * 100);
...
int main(int argc, char* argv[])
{
 ...
 //   if (argv[2] != NULL) { image_name = argv[2]; }
    ...
    */
    ...
    boxes = decode_infer_sigmoid(tensor_scores_host, layers[2].stride, yolosize, INPUT_SIZE, num_classes, layers[2].anchors, threshold);
    result.insert(result.begin(), boxes.begin(), boxes.end());

    boxes = decode_infer_sigmoid(tensor_boxes_host, layers[1].stride, yolosize, INPUT_SIZE, num_classes, layers[1].anchors, threshold);
    result.insert(result.begin(), boxes.begin(), boxes.end());

    boxes = decode_infer_sigmoid(tensor_anchors_host, layers[0].stride, yolosize, INPUT_SIZE, num_classes, layers[0].anchors, threshold);
    result.insert(result.begin(), boxes.begin(), boxes.end());
