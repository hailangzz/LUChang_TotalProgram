...
std::vector<BoxInfo> decode_infer_sigmoid(MNN::Tensor& data, int stride, const yolocv::YoloSize& frame_size, int net_size, int num_classes,
        ...
        for (int ci = 0; ci < channels; ci++)
        {
            ...
            for (int hi = 0; hi < height; hi++)
            {
                ...
                for (int wi = 0; wi < width; wi++)
                {
                    ...
                    auto confidence = width_ptr[4];

                    for (int cls_id = 0; cls_id < num_classes; cls_id++)
                    {
                        float score = cls_ptr[cls_id] * confidence;

                        //std::cout << "cls_ptr[cls_id]:" << cls_ptr[cls_id] << "confidence:" << confidence << "score:" << score << std::endl;
                        if (score > threshold)
                        {
                            float cx = (width_ptr[0] * 2.f - 0.5f + wi) * (float)stride;
                            float cy = (width_ptr[1] * 2.f - 0.5f + hi) * (float)stride;
                            float w = pow(width_ptr[2] * 2.f, 2) * anchors[ci].width;
                            float h = pow(width_ptr[3] * 2.f, 2) * anchors[ci].height;
