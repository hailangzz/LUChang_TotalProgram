...
class SUBM(nn.Module):
    ...
    def __init__(self, alpha=0.9):
        super(SUBM, self).__init__()
        self.alpha = alpha

    def forward(self, x):
        """ print("***********************")
        """
        return torch.sub(x[0],x[1],alpha=self.alpha)
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=13, anchors=(), ch=()):  # detection layer
        ...
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv  # 注：网络系统的检测输出端，按照输出侧的设定输出层特征数，创建空的卷积Conv2d模型数组，将用于检测输出时的计算功能
        # ch ：检测输出层的特征通道数：[128, 256, 512]
    def forward(self, x):
        '''
        ####################################################
        # 修改网络最后代码，将npu不支持的屏蔽掉
        z = []  # inference output
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = x[i].sigmoid()  # 此处为将目标检测的输出值全部进行sigmoid概率计算，其中包括目标分类、前景背景、box_value值等
                #print("**")
                #print(y.shape) #[1, 3, w, h, 85]
                #print(self.grid[i].shape) #[1, 3, w, h, 2]
                # 以下为进一步对计算sigmoid计算完的检测框值，进行反向运算，对检测框的中心点、尺寸信息进行反向还原。
