...
from lib.models.common import Conv, SPP, Bottleneck, BottleneckCSP, Focus, Concat, Detect, SharpenConv,C3,SPPF,SUBM
...
[ [-1, 10], Concat, [1]],   #22
[ -1, BottleneckCSP, [512, 512, 1, False]],     #23
[ [17, 20, 23], Detect,  [1, [[3,9,5,11,4,20], [7,18,6,39,12,31], [19,50,38,81,68,157]], [128, 256, 512]]], #Detection head 24  检测类别数:1 自定义 anchors:[[3,9,5,11,4,20], [7,18,6,39,12,31], [19,50,38,81,68,157]] 特征通道数：[128, 256, 512]
...
def get_net(cfg, **kwargs): 
    # m_block_cfg = YOLOP
    m_block_cfg = YOLOP_MY2
    model = MCnet(m_block_cfg, **kwargs)
    # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model = MCnet(m_block_cfg, **kwargs).to('cpu')  # 模型的设备加载 cpu、gpu
    return model
...
if __name__ == "__main__":
    ...
    detects_out,lane_line_seg = model(input_)

    summary(model, input_size=(3, 256, 256),device="cpu")

    # print(len(model_out))
    # print(model_out[0][0].shape,model_out[0][1].shape,model_out[0][2].shape)
    # model_out,SAD_out = model(input_)
    # detects, dring_area_seg, lane_line_seg = model_out
    # Da_fmap, LL_fmap = SAD_out
    for det in detects_out:
        print(det.shape)
